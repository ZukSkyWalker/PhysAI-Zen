---
layout: post
title: "Maximum-Entropy RL – Langevin Dynamics + Maximum Caliber"
date: 2025-12-10
author: ZukSkyWalker
...
![Langevin Dynamics: Temperature Controls Exploration]({{ '/plots/langevin_dynamics_temperature_controls.png' | relative_url }})
...
## 2. Maximum Caliber: Entropy Over Paths

### 2.1 Jaynes' Maximum Entropy Principle (Static)
...
### 3.3 Connection to Langevin Dynamics
...
## 4. Algorithm: Soft Actor-Critic (SAC)
...
![Boltzmann Policy: Temperature Controls Exploration-Exploitation]({{ '/plots/Boltzmann_Policy.png' | relative_url }})
...
## 5. ML Narrative: Why MaxEnt Wins

### 5.1 Comparison with Standard RL

| Standard RL | MaxEnt RL (SAC) |
|-------------|-----------------|
| Policy: deterministic or ε-greedy | Policy: stochastic Boltzmann |
| Exploration: external (e.g., noise injection) | Exploration: intrinsic (entropy bonus) |
| Bellman equation: Q(s,a) = r + γ max Q(s',a') | Soft Bellman: Q(s,a) = r + γ E[softmax Q(s',·)] |
| Overestimates value (max bias) | Less biased (soft max) |
| Brittle to environment changes | Robust (multi-modal policy) |

### 5.2 Empirical Advantages
...
### 5.3 Training Results on Pendulum-v1

The figure below shows SAC training on the continuous control task Pendulum-v1 (swing-up and balance):

![SAC Training on Pendulum-v1]({{ '/plots/RL_training_log.png' | relative_url }})
...
### 5.4 Learned Policy and Value Function
...
![Policy Actions and Q-Values in State Space]({{ '/plots/policy_and_Q.png' | relative_url }})
